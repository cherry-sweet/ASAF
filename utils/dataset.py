...
class CUB(VisionDataset):
	...
	base_folder1 = 'CUB_200_2011/image_vit'
	...
	def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
		super(CUB, self).__init__(root, transform=transform, target_transform=target_transform) ###  在父类里面进行赋值了
	...
	def __getitem__(self, idx):
		...
		if self.train:
			path1 = os.path.join(self.root, self.base_folder1, sample.filepath)
...
class Cars(VisionDataset):
	...
	def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
		...
		if self.train :
			...
			self.image_path = os.path.join(root, 'cars_train')
		else:
			list_path = os.path.join(root, 'devkit', 'cars_test_annos_withlabels.mat')
			self.image_path = os.path.join(root, 'cars_test')
		list_mat = scipy.io.loadmat(list_path)
		self.samples = []
		length = len(list_mat['annotations']['fname'][0])
		for i in range(length):
			path = str(list_mat['annotations']['fname'][0][i].item())
			label = int(list_mat['annotations']['class'][0][i].item() - 1)
	...
	def __getitem__(self, index):
		...
		if self.train:
			path = os.path.join(self.root, 'cars_train', path)
			path1 = os.path.join(self.root, 'image_vit', path)
		else:
			path = os.path.join(self.root, 'cars_test', path)
...
class Dogs(VisionDataset):
	...
	def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
		...
		self.images_folder1 = join(self.root, 'image_vit')
	...
	def __getitem__(self, index):
		...
		image_path1 = join(self.images_folder1,image_name)
