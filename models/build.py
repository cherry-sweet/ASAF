...
def baseline_models(config, num_classes):
	...
	if type == 'resnet':
		...
		if config.model.name.lower() == 'swin tiny':
			...
			#                                  num_classes=num_classes, drop_path_rate=config.model.drop_path)
			model = swin_backbone(num_classes=num_classes, drop_path_rate=config.model.drop_path,
			                         img_size=config.data.img_size, window_size=config.data.img_size // 32)
	elif type == 'maxvit':
...
def load_pretrained(config, model, nn=None):
	...
	# 	del state_dict['fc.bias']
		...
		# delete relative_position_index since we always re-init it   # relative_position_index 相对位置编码机制
		...
		if not config.model.name.lower() == 'swin tiny':
